...
class FirestoreDatabase:
    ...
    def teams(self):
        ...
        teams: list[TeamEntry] = [
            TeamEntry(doc.id, doc.to_dict()["tg_group_id"], set(doc.to_dict()["members"]))
            for doc in docs
        ]
    ...
    def get_team(self, team: (str, TeamEntry)):
        doc_ref = self.db.collection("teams").document(str(team))
        doc = doc_ref.get()
    ...
    def set_team(self, team: TeamEntry):
        doc_ref = self.db.collection("teams").document(str(team))
        team_data = {"members": team.members, "tg_group_id": team.tg_group_id}
    ...
    def get_question(self, question: (str, QuestionEntry)):
        questions_ref = self.db.collection("questions")
        questions_query = questions_ref.where("question", "==", str(question)).stream()
        q_item = None
        ...
        if q_item:
            ...
            return QuestionEntry(
                question_dict["key"], question_dict["question"], question_dict["answer"]
            )
        else:
            return None

    def set_question(self, question: QuestionEntry):
        self.db.collection("questions").document().set(question.to_dict())
    ...
    def chats(self):
        chats_list = self.db.collection("chats").get()
        chats = []
        ...
        for chat_doc in chats_list:
            ...
            chats.append(
                ChatEntry(
                    chat_doc.id,
                    messages=[
                        ChatMessage(m["message"], m["user_sent"], m["time"])
    ...
    def get_chat(self, chat_id: (str, ChatEntry)):
        doc_ref = self.db.collection("chats").document(str(chat_id))
        doc = doc_ref.get()
    ...
    def set_chat(self, chat_entry: ChatEntry):
        doc_ref = self.db.collection("chats").document(chat_entry.chat_id)
        team_data = {
            "messages": [m.to_dict() for m in chat_entry.messages],
            "is_open": chat_entry.is_open,
        }

        doc_ref.set(team_data)

    def delete_chat(self, chat_entry: (str, ChatEntry)):
        doc_ref = self.db.collection("chats").document(str(chat_entry))
        doc_ref.delete()
